                                ...
                                <el-row :class="[i2===0?'':'bdtop','vcenter']" v-for="(item2,i2) in item1.children"
                                    ...
                                    <el-col :span="18">
                                        <el-tag closable type="warning" v-for="(item3,i3) in item2.children"
                                            :key="item3.id" @close="removeRightsById(item3.id,scope.row.id)">
                                            {{item3.authName}}
                                        </el-tag>
                                    </el-col>
                                </el-row>
...
<script>
    export default {
        ...
        methods: {
            ...
            // 根据ID删除权限
            async removeRightsById(rightsId, roleId) {
                // 弹框确定是否删除
                ...
                console.log(rightsId + "/" + roleId);
                const res = this.$http.delete("roles/" + roleId + "/rights/" + rightsId);
                console.log(res);
                return this.$message.success("删除成功");
            }
        }
